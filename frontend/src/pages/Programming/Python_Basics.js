...
_capture = FastCapture()
  ...
  const fetchUserAnsweredQuestions = async (email) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/finger-exercise?email=${email}&topic=python_basics`, {
        credentials: 'include'
      });
  ...
  const initPracticeQuiz = async (answeredIds) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/finger-questions?topic=python_basics`, {
        credentials: 'include'
      });
