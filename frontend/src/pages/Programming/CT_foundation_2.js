...
const CTFoundation2 = () => {
  ...
  const initializeQuiz = async (email) => {
    try {
      const response = await fetch('https://3.111.49.131:4000/api/CT_finger?topic=CT_foundation1');
  ...
  const fetchPreviousAnswers = async (email) => {
    try {
      const response = await fetch(`https://3.111.49.131:4000/api/CT_finger_scores/${email}/CT_foundation_1`, {
        credentials: 'include'
      });
