// src/pages/Programming/programming_questions.js
...
// Pyodide setup and code execution logic
let pyodideInstance = null;
let isInitializing = false;

export const initializePyodide = async () => {
    if (pyodideInstance || isInitializing) return pyodideInstance;
...
def start_capture():
    global _old_stdout, _capture_active
...
def get_captured_output():
    ...
    return ""
        ...
        pyodideInstance.runPython('start_capture()');
        
        try {
            await pyodideInstance.runPython(code);
            rawOutput = pyodideInstance.runPython('get_captured_output()').trim();
        } catch (execError) {
            // Ensure we always release stdout capture
            pyodideInstance.runPython('get_captured_output()');
            return {
        ...
        for (const testCase of testCases) {
            ...
            try {
                // Start fresh capture for each test case
                pyodideInstance.runPython('start_capture()');
                
                let testCode;
                if (testCase.input && testCase.input.length > 0) {
                    const params = testCase.input.map(param => {
                        if (typeof param === 'string') {
                            // Handle string parameters carefully
                ...
                const result = await pyodideInstance.runPython(testCode);
                const output = pyodideInstance.runPython('get_captured_output()').trim();
                
                // Handle both printed output and returned values
                testResult.output = output || String(result);
                testResult.passed = testResult.output === testCase.expected;
                
            } catch (testError) {
                testResult.output = `Error: ${testError.message}`;
...
export const fetchSessionInfo = async () => {
    try {
        const response = await fetch('https://3.111.49.131:4000/api/session-info', {
            credentials: 'include'
        });
...
export const submitDiagnostic = async (userInfo, submissions) => {
    try {
        const response = await fetch('https://3.111.49.131:4000/api/programming/submit', {
            method: 'POST',
