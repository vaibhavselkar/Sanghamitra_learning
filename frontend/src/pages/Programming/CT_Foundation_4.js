...
const CTFoundation4 = () => {
  ...
  const initializeQuiz = async (email) => {
    try {
      const response = await fetch('https://sanghamitra-learnworld.vercel.app/api/CT_finger?topic=CT_foundation_4');
  ...
  const fetchPreviousAnswers = async (email) => {
    try {
      const response = await fetch(`https://sanghamitra-learnworld.vercel.app/api/CT_finger_scores/${email}/CT_foundation_4`, {
        credentials: 'include'
      });
