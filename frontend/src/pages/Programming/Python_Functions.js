...
_capture = FastCapture()
  ...
  const fetchUserAnsweredQuestions = async (email) => {
    try {
      ...
      const response = await fetch(`http://3.111.49.131:4000/api/finger-exercise?email=${email}&topic=python_functions`, {
        credentials: 'include'
      });
  ...
  const initPracticeQuiz = async (answeredIds) => {
    try {
      ...
      const response = await fetch('http://3.111.49.131:4000/api/finger-questions?topic=python_functions', {
        credentials: 'include'
      });
