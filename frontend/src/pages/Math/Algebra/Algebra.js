    ...
    const fetchData = async () => {
      try {
        ...
        const testResponse = await fetch(
          `${process.env.REACT_APP_API_URL}/api/algebra-testresponses?email=${encodeURIComponent(email)}&all=true`,
          { credentials: 'include' }
        );
        ...
        for (const topic of topics) {
          try {
            const response = await fetch(
              `${process.env.REACT_APP_API_URL}/api/algebra-scores?userEmail=${encodeURIComponent(email)}&operationType=${topic.id}`,
              { credentials: 'include' }
            );
